...
| **分类 + 识别合并调用** | 最多 **8 张图**走合并调用（`<<<PAGE n\|NEW/CONT>>>` 分隔符协议），**1 次请求拿回"题型 + 全部逐页转录"**（旧行为 9 次），实测 8 图 ≈7.7 s；超过 8 张的组按 `VISION_BATCH_SIZE`（默认 8）分批并发。图片 token 只按一次计费。任一页缺失走单页补做，完全解析不了才回退 JSON 协议 → 分类 + 并行 OCR |
| **题目文本本地排版** | 合并路径不调润色模型，因此排版由 `text_layout.normalize_pages()` 本地完成（零 token）：剥离 App 页眉/页脚/按钮行但**保留题号与题型**（`单选题 第18/60题 自动跳下一题` → `单选题 第18题`）、合并被翻页切开的代码围栏、去掉 CONT 页的重抄段、规整空行 |
| **只把公式送去规范化** | 排版解决不了的"公式没被 `$...$` 包住"由一次**轻量**调用处理：抽出公式片段 → 送模型 → 填回原位，输出量几十 token（而润色重写整篇约 10K），**正文一个字符都不经过模型**（`FORMULA_NORMALIZE`） |
