...
# 按题型定制的排版要求（由 `core_pipeline._textualize` 注入润色 prompt 的 {type_hint}）。
#
# 为什么需要：题型在**分类 + 转录那一次调用**里就已经得到了（`problem_type`），
# 而排版规则本身是**依赖题型**的 —— 选择题要把选项一段一段分开、编程题要把代码放进
# 围栏、填空题要保住下划线占位符。原先润色请求只拿到"原始文本"，只能用一套通用规则，
# 等于白白浪费了一个已知信息。**注入这段不产生任何额外请求。**
_TYPE_FORMAT_HINTS: dict[str, str] = {
    "MULTIPLE_CHOICE": (
        "本题型是**选择题**（含多选）。排版重点：\n"
        "- **每个选项独占一段并用空行分隔**（`A. …` 空行 `B. …` 空行 …），绝对不能挤成一段；\n"
        "- 题号与题干保持完整，选项标号 A/B/C/D 一个都不能少、不能合并；\n"
        "- 选项里若带代码，代码用围栏代码块包住；\n"
        "- 题干里的括号占位符 `（ ）` / `()` 必须原样保留。"
    ),
    "FILL_IN_THE_BLANKS": (
        "本题型是**填空题**。排版重点：\n"
        "- **下划线占位符（`____`）、方括号占位符（`[ ]`）、`（ ）` 必须原样保留**，\n"
        "  不得补全、不得填成答案、不得改写宽度 —— 它们是题目的作答位置；\n"
        "- 题干与每个空所在的句子分段清晰；\n"
        "- 多个空按原顺序保留编号（如 `(1)` `(2)`）。"
    ),
    "CODING": (
        "本题型是**编程题**。排版重点：\n"
        "- 题目描述、输入/输出示例、约束条件各自独立成段；\n"
        "- **所有多行代码一律放进围栏代码块并标注语言**（```python 等），不要保留 OCR 行号；\n"
        "- 函数签名、变量名、边界条件与数值**逐字不变**；\n"
        "- 示例的「输入 / 输出 / 解释」用小标题 `###` 或加粗标签标出，层次清楚。"
    ),
    "ML_CODING": (
        "本题型是**机器学习编程题**。排版重点：\n"
        "- 数据说明、要求实现的模型/指标、评价方式各自独立成段；\n"
        "- **所有代码放进围栏代码块并标注语言**（```python 等），不要保留 OCR 行号；\n"
        "- 公式（损失函数、梯度、指标定义）必须用 LaTeX 包裹，块级公式 `$$` 独占一行；\n"
        "- 维度、超参数、数值**逐字不变**。"
    ),
    "VISUAL_REASONING": "本题型是**图形推理题**。文字说明按段落排清即可，重点保证题干完整。",
    "QUESTION_ANSWERING": (
        "本题型是**问答题**。排版重点：\n"
        "- 材料/上下文与问题分开，材料可用引用块 `>` 或独立小节；\n"
        "- 问题单独成段，层次清楚。"
    ),
    "GENERAL": (
        "本题型是**通用文字题**。排版重点：\n"
        "- 按段落整理，题干与设问分开；\n"
        "- 若有小题编号（`1.` `2.`）保留编号并按列表排版。"
    ),
}


def format_hint_for(problem_type: str | None) -> str:
    """取该题型的排版要求；未知题型给通用兜底。

    不返回空串：prompt 里 `{type_hint}` 是独立一节，留空会让模型以为"没有要求"，
    反而不如明确给"按通用规则排版"。
    """
    return _TYPE_FORMAT_HINTS.get((problem_type or "").strip().upper(), _TYPE_FORMAT_HINTS["GENERAL"])


def build_polish_prompt(raw_texts: str, problem_type: str | None = None) -> str:
    """拼出"合并 + 排版"的完整 prompt。

    题型由第 1 阶段（分类 + 转录，同一次请求）给出，这里只做**文本注入**，
    不产生任何额外请求。用 `replace` 而不是 `format`：题目里的花括号（如 `{ }`）
    不该被当成占位符。
    """
    return (
        TEXT_MERGE_AND_POLISH_PROMPT
        .replace("{type_hint}", format_hint_for(problem_type))
        .replace("{raw_texts}", raw_texts)
    )
...
# 本题型的排版要求（来自上游分类结果，务必遵守；不需要你重新判断题型）
{type_hint}
...
# 第三步：Markdown 排版（人要直接看**渲染结果**读题，这一步必须做好）
输入是截屏 OCR 的原始片段，直接渲染会很难看：正文不换行、选项挤成一坨、
代码被行号列表包住。因此合并之后必须重新排版：

1. **段落之间空一行**。Markdown 里连续两行会渲染成同一段 —— 题干、每个选项、
   每个代码块之间都要有空行，否则渲染后挤成一坨。
2. **选项各自独占一行并空行分隔**（`A. …` / `B. …` 各占一段），不要挤在一个段落里。
3. **多行代码放进围栏代码块**，不要保留 `1.` `2.` `3.` 行号 —— Markdown 会把它渲染成
   有序列表，长行还会被缩进折行，非常难读。语言能判断就标注（```python / ```java），
   判断不了就用裸围栏；围栏**必须成对**。
   注意：题目内容本身要求的"第 1 步/第 2 步"要照抄，不要当行号删掉。
4. **修掉 OCR 造成的错误换行**：一句话被从中间截断时（上一行以逗号、"的/和/与"等结尾）
   合成一行；**但代码行之间不要合并**。
5. **小标题**（如"题目一""第 1 部分"）用 `###`，不要用加粗大段文字。
6. **删掉与题目无关的截屏噪音**：App 的科目栏、试卷编号、`满分：.. 及格：.. 已答`、
   `第 x/y 题` 进度、`上一题 下一题 存疑` 这类按钮行。
   **但题号与题型（单选/多选/判断/填空…）必须保留** —— 读题的人首先要知道这是第几题、
   什么题型。页眉行里混着它们时（如 `单选题 第18/60题 自动跳下一题`），
   只去掉进度与按钮，规范成 `单选题 第18题`。
7. 输出**不要**再包含 `---[NEXT]---` —— 它只是输入的片段边界。
8. 不要用代码围栏把**整篇**输出包起来。
...
# ------------------------------------------------------------------------------
# 公式规范化（C：只把公式送去模型，而不是重写整篇文本）
#
# 为什么需要：合并路径走本地拼接、**不调润色模型**（见 core_pipeline._textualize），
# 于是 `TEXT_MERGE_AND_POLISH_PROMPT` 里那条最重要的"数学公式规范化"从来没被执行：
# 行内公式没包 `$...$`、`\begin{}` 环境没包 `$$...$$`、`$` 不成对 —— 解答文件里的
# 公式因此可能不渲染。
#
# 为什么不让润色模型重写整篇：那要输出 10K token（整条链最贵的一次），且它的
# "找最长重叠"指令在多道独立题场景会**静默删掉一道题的开头**。
#
# 这里的做法：本地抽出公式片段 → 只把公式送去规范化 → 填回原位。输出量从
# 10K token 降到几十 token，且**正文一个字都不经过模型**，不存在被删改的风险。
# ------------------------------------------------------------------------------
FORMULA_NORMALIZE_PROMPT = r"""
你是 LaTeX 公式规范化器。下面给你一批**从 OCR 文本里抽出来的数学片段**，编号从 0 开始。

# 任务
逐条把片段规范化成正确的 LaTeX，并**只输出一个 JSON 数组**，元素个数与输入完全一致。

# 规则
1. **数量绝对一致**：输入 N 条就必须输出 N 条，顺序一一对应。不许合并、拆分、增删，也不许跳过任何一条。
2. **不改变数学含义**：只做格式修正，不得改写数值、符号、变量名或表达式结构。
3. **分隔符**：块级环境（`\begin{...}...\end{...}`、矩阵、多行公式）保留/补全 `$$...$$`；
   单行公式与变量用 `$...$`。**不要**输出 `\[` `\]` `\(` `\)` 这几种分隔符，统一换成 `$`/`$$`。
4. **反斜杠原样保留**：`\frac` `\begin` `\theta` `\neq` `\sqrt` 等必须是标准 LaTeX 命令，
   不得退化成普通字符（例如 `\frac` 不许变成 `rac` 或 formfeed）。
5. **修明显的 OCR 错误**：如 `\f rac` → `\frac`、`\thet a` → `\theta`、缺失的 `{}`/`_`/`^` 补上花括号。
   拿不准的**保持原样** —— 保持原样永远比猜错安全。
6. 片段本身不是公式（是普通文字）时，**原样返回该字符串**。

# 输出格式
只输出 JSON 数组，不要 Markdown 代码块，不要任何解释文字。示例：
["$x=1$", "$$\\begin{bmatrix}a&b\\\\c&d\\end{bmatrix}$$", "普通文字"]

# 待规范化片段（JSON 数组，编号即下标）
{formulas_json}
"""
