...
""",
    ...
    "ML_CODING": {
        # 对于ML手撕题，"探索式"和"最优解"通常是同一个目标：一个清晰、正确、标准的实现。
        # 因此，我们让两种风格都使用这个强大的新模板。
        "OPTIMAL": ML_CODING_PROMPT,
        "EXPLORATORY": ML_CODING_PROMPT
    },
...
# CoT (Chain of Thoughts) - 思维与输出框架
### 1. 题目分析与直觉思路
*   **任务**: 用最直白的方式解释题目要求，提出一个逻辑清晰、易于实现的解法思路（例如，一个直观的暴力解法或简单的贪心策略），再明确给出该解法的时间和空间复杂度，并分别用一句话简述其原因。
...
# CoT (Chain of Thoughts) - 思维与输出框架
### 1. 题目分析与核心思路
*   **任务**: 在此部分，提炼本题的所有要求，并提出一个虽然不一定最优，但逻辑清晰、确保能得到正确答案的算法思路。再明确给出该解法的时间和空间复杂度，并分别用一句话简述其原因。
