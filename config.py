...
你的任务是为下面的LeetCode风格编程题目提供一份“超越标准答案”的深度教学式题解，聚焦于最高效的解决方案。
你必须严格遵循下面指定的结构进行回答，并使用提供的Markdown标题。
...
{solution_plan}
---
**重要:** 此计划是一个高层指导。在开始前，你必须首先完成下面的“需求分析”步骤，并以“问题文本”中的原始需求为最高准则来验证和调整计划，确保最终解法完全符合题意。

### 1. 需求分析 (Requirement Analysis)
*   **核心任务:** 仔细阅读“问题文本”，明确指出本题的核心计算任务是什么？（例如：求最短路径、统计频率、计算特定统计量等）
*   **输入/输出格式:** 详细描述输入数据的格式、约束条件以及输出结果的精确格式要求。
*   **关键细节与边界:** 列出所有必须处理的特殊规则、数值格式要求或边界条件。

### 2. 核心思路与算法选择
*   **解题直觉:** 基于完整的需求分析，用一两句话描述解决这个问题的直观想法。
...
### 3. 更优方案探讨
*   **可能的优化:** 简要提及存在一个或多个更优的解法，并指出它们能够优化的性能。
...
""",
        # The entire 'OPTIMAL' prompt for ACM is rewritten for robustness.
        "OPTIMAL": """
...
{solution_plan}
---
**至关重要:** 以上计划仅为初步构想。你的首要任务是完成下面的“需求分析”，并以此为唯一标准来验证、修正或完全重构计划。最终代码必须100%满足原始问题文本的所有要求。

### 1. 需求分析 (Requirement Analysis)
*   **核心任务:** 仔细阅读“问题文本”，一字不差地明确本题要求计算并输出的所有内容。例如，是求和、求平均值，还是一个包含多个统计量的列表？
*   **输入/输出格式:** 精确描述输入数据的格式（例如，多行输入、逗号分隔）、所有变量的范围和约束。同样，精确描述输出结果的格式要求，包括数值格式（整数、特定位数小数、四舍五入规则）。
*   **特殊规则与边界:** 列出所有必须处理的特殊情况，例如空输入、数组长度小于窗口大小等情况的处理方式。
...
任务完成后，请另起一行，并严格遵循 'TITLE: [5-10个字的中文解法总结]' 的格式，为该题解生成一个适合用作文件名的标题。
""",
        # The entire 'EXPLORATORY' prompt for ACM is rewritten for robustness.
        "EXPLORATORY": """
...
提供一份完整的、可独立运行的、基于你上述“稳健”思路的Python脚本，确保其完全符合“需求分析”的结论。
