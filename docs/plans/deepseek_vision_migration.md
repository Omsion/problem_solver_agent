...
> **⚠️ 本节的"批量 4"已于 2026-09-23 被 §8.8 推翻**：那次比较的是"合并路径 vs 并行
> 路径"，**没有**比较批量 4 与批量 8。§8.8 的交替轮次实测显示批量 4 是 8.68 s /
> 3 次请求 / 每轮补页，而批量 8 是 7.68 s / 1 次请求 / 0 补页 —— 默认值因此改为 8。
> 下面这条"保持分批合并（成本优先）"的决策指的是**不用并行回退路径**（省 60% 成本），
> 那个结论仍然成立；它说的不是"批量取 4"。
...
### 8.8 `VISION_BATCH_SIZE` 该取多少（2026-09-23 实测，**推翻 8.6 的 4**）

用户提问："4 是什么经验值？改大能省 token 吗？能直接设 16 吗？"为此新增探针
`_probe/probe_batch_sizes.py`（交替轮次，同时记录**真实请求数**与**每次请求带了几张图**）。

**先厘清性质：这个参数与 token 成本无关。** 官方规则是"每张图独立计费、上限
1024 token/图"，而分批**不重传**图片（`_merge_batch_results` 按下标写回同一份页
列表）。因此 token 只取决于"同一张图被上传几次"，与批量大小无关 ——
测得的 `图片张次` 在批量 4/8/16 下都等于图片数，可以直接核对。
受影响的是三件事：**请求数**（每次请求重发 prompt 模板）、**单请求输出长度**
（越长越容易漏页）、延迟。

#### 8 张图（`_probe/ab_images`，3 轮交替）

| 方案 | 中位数 | 请求数 | 图片张次 | 补页 |
|---|---|---|---|---|
| 批量 4（当时默认） | **8.68 s** | 3 | 9 | 每轮 1 页 |
| 批量 8（1 次请求） | **7.68 s** | 1 | 8 | 0 |
| 批量 16 + 闸门抬到 16 | 8.02 s | 1 | 8 | 0 |
| 批量 16 但闸门仍是 8 | 8.18 s | 1 | 8 | 0 |

后三行**完全相同**是必然的：只有 8 张图，批量 8/16 都只切出 1 批，`COMBINED_VISION_MAX_IMAGES=8`
是闸门、不是批量。**批量 4 在这三个维度上都不占优**（更慢、请求更多、还触发补页），
§8.6 里"分批更快"的前提到此不成立。

> **默认值已按本节结论改为 8（2026-09-23，用户确认）**：`VISION_BATCH_SIZE` 由 4 改 8。
> 4 当初的依据（§8.2 成块测量"分批 2×4 比一次带 8 张快 6.4 s vs 8.4 s"）是负载噪声的产物，
> 交替轮次复测把结论反了过来。**改动只影响超过 8 张的组**：≤8 张时两版都是 1 批 1 请求。

#### 16 张图（`_probe/ab16`，把 8 张复制一份凑成 16 张；2 轮交替）

| 方案 | 中位数 | 请求数 | 单请求最多图 | 图片张次 | 非空页 |
|---|---|---|---|---|---|
| 批量 4，闸门 8 | **0.00 s（0 请求）** | 0 | — | 0 | 0/16 |
| 批量 8，闸门 8 | **0.00 s（0 请求）** | 0 | — | 0 | 0/16 |
| 批量 16，闸门 16 | 20.37 s | 10 | 16 | **25** | 16/16 |
| 批量 16，闸门 8 | **0.00 s（0 请求）** | 0 | — | 0 | 0/16 |

这张表回答了"能不能直接设 16"：**不能，而且后果比"没效果"更差**。

1. **闸门卡的是总图片数，且它在分批之前**。`combined_call_enabled(len(image_paths))`
   在 `> COMBINED_VISION_MAX_IMAGES` 时直接返回 None —— 于是总图数 >8 的组
   **一次合并请求都不发**，整组回退到"1 次分类 + N 次逐页 OCR"（图片被上传两次）。
   实测批量 4/8/16 在闸门=8 时**全部 0 请求**，就是这个机制。
2. **抬闸门 + 大批量能走通，但模型明显漏页**：批量 16 一次带 16 张，16 页里
   **9 页要单页补做**（补做是真金白银的额外请求，最终 10 次请求 / 25 张次），
   而补做的直接原因是响应太长、PAGE 块没写全。
3. 因此"把 `VISION_BATCH_SIZE` 设成 16"这件事本身没有意义 —— 要么被闸门挡回并行路径，
   要么走通但把漏页率抬上去。要处理 10 张以上的组，正确的做法是**同时**抬
   `COMBINED_VISION_MAX_IMAGES`，并接受补做；或者干脆让 8 秒窗口自然把大组分小。

**结论（已回填 `.env.example` 与 `config.py` 注释）**：
- 参数性质：与 token 无关；只影响请求数 / 漏页风险 / 延迟。
- `4` 不是"最省 token"的值（没有那种值）—— 它是 §8.2 那次成块测量的产物，
  而交替轮次复测显示它在延迟、请求数、补页率上**都不占优**。
- **默认值改为 `8`**（2026-09-23，用户确认）：≤8 张的组仍是 1 批 1 请求（与旧行为一致），
  只有 >8 张的组从"4 批 N 次请求"变成"ceil(N/8) 批"。
- `16` 需要同时抬闸门，且实测漏页 9/16 —— **不采用**。
