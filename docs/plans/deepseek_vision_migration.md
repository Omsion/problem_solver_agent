...
§8.6 里"分批更快"的前提到此不成立。默认值仍保留 4，理由是它**更保守**
（单请求输出更短 ⇒ 漏页风险更低），而不是它更快。
...
**结论（回填 `.env.example` 与 `config.py` 注释）**：
- 参数性质：与 token 无关；只影响请求数 / 漏页风险 / 延迟。
- `4` 不是"最省 token"的值（没有那种值），它是**保守值**；实测 `8` 在延迟、请求数、
  补页率上都不差，想快可改 8（一行）。
- `16` 需要同时抬闸门，且实测漏页 9/16 —— 不推荐作为默认。
