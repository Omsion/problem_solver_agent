...
core 只上报**可观测的事实**（阶段、模型、页数、输出字符数、调用次数、实际上传图片数），
不猜 token 数。本模块负责把这些事实估算成 token 并落库。

为什么是估算：标准 OpenAI SDK 只有流式响应结束后才能从 `usage` 字段取到精确
token；并非所有兼容网关都返回该字段。因此这里用保守的启发式换算，
目的是**限制滥用**，而不是出精确账单。

换算规则（三个量各自独立，不可互相替代 —— 详见 `estimate_tokens` 的说明）：
- 固定开销（prompt 模板 + 上下文）：按**调用次数**重复；
- 图片：按**实际上传张数**计（`images`，不是"页数 × 次数"）；
- 输出：字符数 × 每字符 token 数，同样按调用次数重复。
"""
...
# **单次调用**的固定输入开销（prompt 模板 + 上下文）。调用 N 次就重复 N 份，
# 因为每次请求都要把模板与上下文重发一遍 —— 见 `estimate_tokens`。
