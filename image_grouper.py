...
4.  **智能工作流**: 编排一个多分支的AI流水线：
    ...
    b. (Agent) 智能分流与策略选择：根据分类结果和代码规则，决定最终的处理路径和提示词模板。
    c. (AI Models) 问题求解：采用“单次调用链式思考”策略，调用相应AI模型一次性解决问题。
5.  **健壮的文件操作**: 在处理完图片后，使用带延时重试的机制将其归档。
6.  **并行任务处理**: 采用生产者-消费者模式，由后台工作线程并行处理任务。
"""
...
class ImageGrouper:
    ...
    def _move_file_with_retry(self, src: Path, dest: Path, retries=3, delay=0.5):
        ...
        for i in range(retries):
            try:
                ...
                if i < retries - 1:
                    logger.warning(
                        f"移动 '{src.name}' 失败 (尝试 {i + 1}/{retries})。将在 {delay}s 后重试... 错误: {e}")
                    time.sleep(delay)
                else:
    ...
    def _execute_pipeline(self, group_to_process: List[Path]):
        ...
        if lock_file_path.exists():
            logger.warning(f"发现锁文件，跳过处理: {lock_file_name}")
            return
        ...
        try:
            ...
            if problem_type == "VISUAL_REASONING":
                ...
                if not prompt_template:
                    ...
                    return
                logger.info(f"策略选择完成: '{problem_type}'。")
            ...
            with open(solution_path, 'w', encoding='utf-8') as f:
                f.write(f"Processed Image Group:\n- " + "\n- ".join(p.name for p in group_to_process) + "\n\n")
                f.write("=" * 50 + "\n\n")
