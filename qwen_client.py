# -*- coding: utf-8 -*-
"""
qwen_client.py - Qwen-VL (DashScope) API 客户端 (V2.2 - 多模型版)

V2.2 版本更新:
- 新增 `solve_visual_reasoning_problem` 函数，专门用于调用更强大的
  `qwen3-vl-thinking` 模型来处理视觉推理任务。
- 重构所有API调用，以支持 `system` 和 `user` 角色的结构化提示词。
"""
...
def _call_qwen_api(image_paths: List[Path], prompt_template: Dict[str, str], model_name: str,
                   extra_params: Dict = None) -> Union[str, None]:
    if not qwen_client:
...
def classify_problem_type(image_paths: List[Path]) -> str:
    ...
    response = _call_qwen_api(image_paths, config.CLASSIFICATION_PROMPT, config.QWEN_MODEL_NAME)
...
def transcribe_images_raw(image_paths: List[Path]) -> Union[List[str], None]:
    ...
    def transcribe_single(index, path):
        return index, _call_qwen_api([path], config.TRANSCRIPTION_PROMPT, config.QWEN_MODEL_NAME)
